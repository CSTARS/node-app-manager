...
function getLatestNodeVersion(callback) {
    request(nodejsUrl, function (error, request, body) {
        ...
        for( var i = 0; i < lines.length; i++ ) {
            if( lines[i].match(/.*node-.*-x86\.tar\.gz.*/) ) {
                callback(lines[i].replace(/.*node-/,'').replace(/-x86\.tar\.gz.*/,'').replace(/-.*\r/,''));
                callback = null;
                return;
            }
        }
...
function downloadLatestNodeVersion(callback) {
    getLatestNodeVersion(function(version){
        ...
        console.log('Downloading '+nodejsUrl+'node-'+version+'-'+nodejsPlatform+'-x86.tar.gz ...');
        request(nodejsUrl+'node-'+version+'-'+nodejsPlatform+'-x86.tar.gz', function(){
