...
console.log('mongo platform: '+mongoPlatform); 
   request(mongoUrl, function (error, request, body) {
        if( !callback ) return;
        var re = new RegExp('https://fastdl.mongodb.org/'+mongoPlatform+'/mongodb-'+mongoPlatform+'-x86_64-\\d+.\\d+.\\d+.tgz');
